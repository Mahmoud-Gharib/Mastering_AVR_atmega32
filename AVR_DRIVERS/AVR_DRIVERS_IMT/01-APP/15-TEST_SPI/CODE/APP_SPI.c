...
#include "../../../03-MCAL/06-SPI/SPI_INTERFACE.h"
...
void APP_voidTestSPISend(void)
{
	...
	while(1)
	{
		...
		MSPI_voidSendData(Local_u8Number);
		_delay_ms(100);
	}
}
void APP_voidTestSPIRcv(void)
{
	HLCD_voidInit();
	MSPI_voidInit();
	...
	while(1)
	{
		Local_u8Number = MSPI_u8ReciveData();
		if(Local_u8Number != 0)
		{
